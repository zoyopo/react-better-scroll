...
const BetterScrollList:BetterScrollType = (props: Iprops) => {
  ...
  const _initPullDownRefresh = () => {
    ...
    scrollInstance.on('leaveThreshold', () => {
      setPullInfo(prev => ({ ...prev, reachRefreshRegion: true }));
    });
  ...
  useImperativeHandle(bscrollListRef, () => ({
    ...
    scrollTo() {
      bscrollInstance.current &&
        // @ts-ignore
        bscrollInstance.current.scrollTo.apply(
          bscrollInstance.current,
          arguments
        );
    },
    scrollToElement() {
      bscrollInstance.current &&
        // @ts-ignore
        bscrollInstance.current.scrollToElement.apply(
          bscrollInstance.current,
          arguments
        );
    },
  ...
  const forceUpdate = (dirty: boolean) => {
    ...
    if (
      (pullDownRefresh || isPullDownEventEnable()) &&
      pullInfo.isPullingDown
    ) {
      setPullInfo(prev => ({
        ...prev,
        isPulling: false,
  ...
  const destroyBscrollList = () => {
    ...
    reboundPullDownTimerRef.current &&
      clearTimeout(reboundPullDownTimerRef.current);
  ...
  const getPullUpText = () => {
    const moreTxt =
      (pullUpLoad && pullUpLoad.txt && pullUpLoad.txt.more) || '加载更多';
    const noMoreTxt =
      (pullUpLoad && pullUpLoad.txt && pullUpLoad.txt.noMore) ||
      '没有更多数据了';
  ...
  const renderDefaultPullDownElement = () => {
    return (
      <>
        ...
        {pullDownRefresh && (
          <div
            className="rc-pulldown-wrapper"
            ref={pulldownWrapperRef}
            style={style['pulldown-wrapper']}
          >
            {pullInfo.beforePullDown ? (
              <div style={style['before-trigger']}>
  ...
  return (
    ...
    >
      <div style={style['scroll-content']}>
        ...
        {PullUpElement ? (
          <PullUpElement isPullUpLoad={pullInfo.isPullUpLoad as boolean} />
        ) : (
