import { ReactElement, useState, CSSProperties, MutableRefObject, useImperativeHandle } from 'react'
import BetterScroll ,{Options}from 'better-scroll'
import { useRef, useEffect } from 'react'
import React from 'react'
import PullLoadBubble from './pullLoadBubble'
type pullUpLoadObj = {
  txt?: {
    more: string
    noMore: string
  }
  threshold:number // 底部距离阈值（threshold）来决定开始加载的时机
}

type pullInfoType = {
  beforePullDown: boolean
  isPullingDown: boolean
  isPulling: boolean
  pullDownStyle?: CSSProperties
  isPullUpLoad?: boolean
  pullDownInitTop?: number
}

type pullDownRefreshObj = {
  txt?: string
  stop?: number // 回弹停留的距离（stop）
  stopTime?: number
  threshold:number // 顶部下拉的距离（threshold）
}
type PullUpElementProps ={
  isPullUpLoad:boolean
}
type PullDownElementProps = {
  beforePullDown:boolean,
   pulldownY:number,
   isPulling:boolean,
   isPullingDown:boolean
}
interface Iprops {
  children: ReactElement
  PullUpElement?:React.FC<PullUpElementProps>
  PullDownElement?: React.FC<PullDownElementProps>
  pullUpLoad?: pullUpLoadObj
  pullDownRefresh?: pullDownRefreshObj
  options?: Options
  probeType?: number
  click?: boolean
  listenScroll?: boolean
  listenBeforeScroll?: boolean
  scrollbar?: object
  startY?: number
  refreshDelay?: number
  freeScroll?: boolean
  scroll?: (pos: any) => void
  beforeScrollStart?: () => void
  onPullingDown?: () => void
  onPullingUp?: () => void,
  bscrollListRef?: MutableRefObject<any>
}



const style:{[props:string]:CSSProperties}= {
  "better-scroll__wrapper":{
    height: '100%',
    width: '100%',
    overflow: 'hidden'
  },
  "scroll-loading": {
    width: '20px',
    height: '20px'
  },
  "pulldown-wrapper":{
    position: 'absolute',
    width: '100%',
    left: 0,
    display: 'flex',
    justifyContent: 'center',
    alignItems: 'center',
    transition: 'all',
    // @ts-ignore
    top:0,
  },
  "after-trigger":{
    marginTop: '10px'
  },
  "pullup-wrapper":{
    width: '100%',
    display: 'flex',
    justifyContent: 'center',
    alignItems: 'center',
    padding: '16px 0'
  }
}

const BetterScrollList = (props: Iprops) => {
  const {
    click = true,
    children,
    PullUpElement,
    PullDownElement,
    pullUpLoad = false,
    pullDownRefresh = false,
    options = {},
    probeType = 1,
    listenScroll = false,
    listenBeforeScroll = false,
    scrollbar = false,
    startY = 0,  
    freeScroll = false,
    scroll,
    beforeScrollStart,
    onPullingDown,
    onPullingUp,
    bscrollListRef,
    
  } = props
  const bubbleRef = useRef(null)
  const wrapperRef = useRef(null)
  const pulldownWrapperRef =useRef<any>()
  const reboundPullDownTimerRef = useRef<any>()
  const afterPullDownTimerRef = useRef<any>()
  // const [isPulling, setIsPulling] = useState(false)
  const [isPullUpToPageEnd,setIsPullUpToPageEnd] = useState(true)
  
  const [pullInfo, setPullInfo] = useState<pullInfoType>({
    isPullUpLoad: false,
    isPulling: false,
    isPullingDown: false,
    beforePullDown: true,
    pullDownStyle: {},
    pullDownInitTop: -50,
  })
  const [bubbleY,setBubbleY] =useState(0)
  const isRebounding = useRef(false)

  const bscrollInstance = useRef<any>()

  useEffect(() => {
    const wrapper = wrapperRef.current
    if(wrapper){
      // @ts-ignore
      wrapper.firstElementChild.firstElementChild.style.minHeight = wrapper.offsetHeight + 'px'
      
    
    }
    setTimeout(() => {
      if(pulldownWrapperRef.current){
        console.log('pulldownWrapperRef',pulldownWrapperRef)
        // @ts-ignore
        pulldownWrapperRef.current.style.top =wrapper.offsetTop + 'px'
      } 
    }, 200);

    setTimeout(() => {
      initScroll()
    }, 20)
  }, [])

  const isPullDownEventEnable = () => {
      return Boolean(onPullingDown)
  }
  const isPullUpEventEnable = () => {
      return Boolean(onPullingUp)
  }
  const initScroll = () => {
    const wrapperDom = wrapperRef.current
    if (!wrapperDom) {
      return
    }
    const _options = Object.assign({}, {
      probeType: probeType,
      click: click,
      scrollY: true, // 下拉刷新组件 只能纵向 不能横向
      scrollX: false,
      scrollbar: scrollbar,
      pullDownRefresh: pullDownRefresh || isPullDownEventEnable(),
      pullUpLoad: pullUpLoad || isPullUpEventEnable(),
      startY: startY,
      freeScroll: freeScroll,
    } ,options) as Object
    bscrollInstance.current = new BetterScroll(wrapperDom, _options)
    if (listenScroll) {
      bscrollInstance.current.on('scroll', (pos:any) => {
        scroll && scroll(pos)
      })
    }

    if (listenBeforeScroll) {
      bscrollInstance.current.on('beforeScrollStart', () => {
        beforeScrollStart&& beforeScrollStart()
      })
    }

    if (pullDownRefresh || isPullDownEventEnable()) {
      _initPullDownRefresh()
    }

    if (pullUpLoad || isPullUpEventEnable()) {
        _initPullUpLoad()
    }
  }
  const _initPullDownRefresh = () => {
    const scrollInstance = bscrollInstance.current
    scrollInstance.on('pullingDown', () => {
      setPullInfo({
        beforePullDown: false,
        isPullingDown: true,
        isPulling: true,
      })
      
      onPullingDown && onPullingDown()
    })
      // v2.4.0 supported
      scrollInstance.on('enterThreshold', () => {
        setPullInfo(prev=>({...prev,isPullingDown:true}))
        
      })
      scrollInstance.on('leaveThreshold', () => {
        console.warn('leaveThreshold')
      
      
      })
      scrollInstance.on('fetching', () => {
        console.warn('fetching')
      })
    if(pullDownRefresh||(isPullDownEventEnable())){
    scrollInstance.on('touchEnd', () => {
    
     setTimeout(() => {
        // @ts-ignore
      bubbleRef.current &&  bubbleRef.current.clearRect()
     
     }, 100);
    
     scrollInstance.on('scrollEnd', () => {
      setTimeout(() => {
        // @ts-ignore
      bubbleRef.current &&  bubbleRef.current.clearRect()
     
     }, 100);
     })
     
    })
    scrollInstance.on('scroll', (pos:any) => {
      if (pullInfo.beforePullDown) {
        setBubbleY(Math.max(0, pos.y + pullInfo.pullDownInitTop))
        //  this.pullDownStyle = `top:${Math.min(pos.y + this.pullDownInitTop, 10)}px`
        setPullInfo((prev) => ({
        pullDownStyle: { top: `${Math.min(pos.y + pullInfo.pullDownInitTop, 10)}px` },
        ...prev,
      }))
      console.log('beforePullDown')
      } else {
        setBubbleY(0)
       
      }

      if (isRebounding.current) {
        setPullInfo((prev) => ({
          // @ts-ignore
          pullDownStyle: { top: `${10 - (pullDownRefresh.stop - pos.y)}px` },
          ...prev,
        }))
      }
    })
  }

  }
  const _initPullUpLoad = () => {
    const scrollInstance = bscrollInstance.current
    scrollInstance.on('pullingUp', () => {
      if (isPullUpToPageEnd) {
        console.log('set isPullUpToPageEnd','true')
        setPullInfo((prev) => ({...prev, isPullUpLoad: true }))
        onPullingUp && onPullingUp()
      }
    })
  }

 
    useImperativeHandle(bscrollListRef,() => ({
      disable() {
        bscrollInstance.current && bscrollInstance.current.disable()
      },
      enable() {
        bscrollInstance.current && bscrollInstance.current.enable()
      },
      refresh() {
        bscrollInstance.current && bscrollInstance.current.refresh()
      },
      scrollTo(){
        bscrollInstance.current &&
        // @ts-ignore
          bscrollInstance.current.scrollTo.apply(bscrollInstance.current, arguments)
      },
      scrollToElement(){
        bscrollInstance.current &&
        // @ts-ignore
         bscrollInstance.current.scrollToElement.apply(bscrollInstance.current, arguments)
      },
      destroy(){
        bscrollInstance.current && bscrollInstance.current.destroy()
      },
      forceUpdate(dirty:boolean){
        forceUpdate(dirty)
      },
      destroyBscrollList(){
        destroyBscrollList()
      }
    }))


  const refresh = () => {
    bscrollInstance.current && bscrollInstance.current.refresh()
  }
 

  // 加载数据后更新
  const forceUpdate = (dirty:boolean) => {
  
      console.log('setIsPullUpToPageEnd',dirty)
      setIsPullUpToPageEnd(dirty)
      if ((pullDownRefresh||isPullDownEventEnable()) && pullInfo.isPullingDown) {
        setPullInfo((prev) => ({ ...prev, isPulling: false }))
        _reboundPullDown().then(() => {
          _afterPullDown()
        })
      } else if (pullUpLoad) {
        setPullInfo((prev) => ({ ...prev ,isPullUpLoad: false, }))
        bscrollInstance.current.finishPullUp()
        refresh()
      } else {
        refresh()
      }
   
  }
  const _reboundPullDown = () => {
    const { stopTime = 600 } = pullDownRefresh as pullDownRefreshObj
    return new Promise((resolve) => {
      reboundPullDownTimerRef.current = setTimeout(() => {
        isRebounding.current = true
        setPullInfo((prev) => ({ ...prev ,isPullingDown: false }))
        bscrollInstance.current.finishPullUp()
        bscrollInstance.current.finishPullDown()
        resolve('')
      
      }, stopTime)
    })
  
  }
  const _afterPullDown = () => {
    afterPullDownTimerRef.current = setTimeout(() => {
      setPullInfo((prev) => ({
        ...prev,
        pullDownStyle: { top: `${pullInfo.pullDownInitTop}px` },
        beforePullDown: true,
      }))
      isRebounding.current = false
      refresh()
    }, bscrollInstance.current.options.bounceTime)
   
  }
  const destroyBscrollList = () => {
    const scrollInstance = bscrollInstance.current
    scrollInstance && scrollInstance.destroy()
    bscrollInstance.current = null
    reboundPullDownTimerRef.current && clearTimeout(reboundPullDownTimerRef.current)
    reboundPullDownTimerRef.current = null
    afterPullDownTimerRef && clearTimeout(afterPullDownTimerRef.current)
    afterPullDownTimerRef.current = null
  }
  const getPullUpText = () => {
    const moreTxt = (pullUpLoad && pullUpLoad.txt && pullUpLoad.txt.more) || '加载更多'
    const noMoreTxt = (pullUpLoad && pullUpLoad.txt && pullUpLoad.txt.noMore) || '没有更多数据了'
    return isPullUpToPageEnd ? moreTxt : noMoreTxt
  }

  const getPullDownText = () => {
    return (pullDownRefresh && pullDownRefresh.txt) || '刷新成功'
  }

  const renderDefaultPullUpElement = () => {
    return (
      <>
        {pullUpLoad && (
          <div style={style['pullup-wrapper']}>
            {pullInfo.isPullUpLoad ? (
               <div style={style['after-trigger']}>
               <img
                 style={style['scroll-loading']}
                 src="data:image/gif;base64,"
               />
             </div>
             
            ) : (
              <div>
                <span>{getPullUpText()}</span>
              </div>
            )}
          </div>
        )}
      </>
    )
  }
  const renderDefaultPullDownElement = () => {
    return (
      <>
        {/* {todo bubble} */}
        {pullDownRefresh && (
          <div className="rc-pulldown-wrapper" ref={pulldownWrapperRef} style={style['pulldown-wrapper']}>
            {pullInfo.beforePullDown? <div style={style["before-trigger"]}>
              <PullLoadBubble bubbleRef={bubbleRef} y ={bubbleY}/>
            </div>
             : (
              <div style={style['after-trigger']}>
               {pullInfo.isPulling? <div>
                  <img
                    style={style['scroll-loading']}
                    src="data:image/gif;base64,"
                  />
                </div> :<div><span>{getPullDownText()}</span></div>}
              </div>
             
            )}
          </div>
        )}
      </>
    )
  }

  return (
    <div className="rc-better-scroll__wrapper" style={style["better-scroll__wrapper"]} ref={wrapperRef}>
      <div style={style['scroll-content']}>
        <div>{children}</div>
       
        {PullUpElement ?<PullUpElement isPullUpLoad={pullInfo.isPullUpLoad as boolean}/>:renderDefaultPullUpElement()}
      </div>
       {PullDownElement?<PullDownElement isPullingDown={pullInfo.isPullingDown} pulldownY ={bubbleY} beforePullDown={pullInfo.beforePullDown} isPulling={pullInfo.isPulling}/>:renderDefaultPullDownElement()}
    </div>
  )
}

export default BetterScrollList
