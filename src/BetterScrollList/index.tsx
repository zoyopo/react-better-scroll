import { ReactElement, useState, CSSProperties, MutableRefObject, useImperativeHandle } from 'react'
import BetterScroll from 'better-scroll'
import styles from './index.less'
import { useRef, useEffect } from 'react'
type pullUpLoadObj = {
  txt: {
    more: string
    noMore: string
  }
  threshold:number // 底部距离阈值（threshold）来决定开始加载的时机
}

type pullInfoType = {
  beforePullDown: boolean
  isPullingDown: boolean
  isPulling: boolean
  pullDownStyle?: CSSProperties
  isPullUpLoad?: boolean
  pullDownInitTop?: number
}

type pullDownRefreshObj = {
  txt?: string
  stop?: number // 回弹停留的距离（stop）
  stopTime?: number
  threshold:number // 顶部下拉的距离（threshold）
}
interface Iprops {
  children: ReactElement
  pullUpElement?: ReactElement
  pullDownElement?: ReactElement
  pullUpLoad?: pullUpLoadObj
  pullDownRefresh?: pullDownRefreshObj
  options?: object
  probeType?: number
  click?: boolean
  listenScroll?: boolean
  listenBeforeScroll?: boolean
  scrollbar: object
  startY: number
  refreshDelay?: number
  freeScroll?: boolean
  scroll?: (pos: any) => void
  beforeScrollStart?: () => void
  onPullingDown?: () => void
  onPullingUp?: () => void,
  bscrollListRef?: MutableRefObject<any>
}

const BetterScrollList = (props: Iprops) => {
  const {
    click = true,
    children,
    pullUpElement,
    pullDownElement,
    pullUpLoad = false,
    pullDownRefresh = {},
    options = {},
    probeType = 1,
    listenScroll = false,
    listenBeforeScroll = false,
    scrollbar = false,
    startY = 0,
    refreshDelay = 20,
    freeScroll = false,
    scroll,
    beforeScrollStart,
    onPullingDown,
    onPullingUp,
    bscrollListRef
  } = props
  const wrapperRef = useRef(null)
  const reboundPullDownTimerRef = useRef(null)
  const afterPullDownTimerRef = useRef(null)
  // const [isPulling, setIsPulling] = useState(false)
  const isPullUpToPageEnd = useRef(true)
  const [pullInfo, setPullInfo] = useState<pullInfoType>({
    isPullUpLoad: false,
    isPulling: false,
    isPullingDown: false,
    beforePullDown: false,
    pullDownStyle: {},
    pullDownInitTop: -50,
  })
  const isRebounding = useRef(false)

  const bscrollInstance = useRef(null)

  useEffect(() => {
    const wrapper = wrapperRef.current
    wrapper.firstElementChild.firstElementChild.style.minHeight = wrapper.offsetHeight + 'px'
    setTimeout(() => {
      initScroll()
    }, 20)
  }, [])
  const initScroll = () => {
    const wrapperDom = wrapperRef.current
    if (!wrapperDom) {
      return
    }
    const _options = Object.assign({}, options, {
      probeType: probeType,
      click: click,
      scrollY: true, // 下拉刷新组件 只能纵向 不能横向
      scrollX: false,
      scrollbar: scrollbar,
      pullDownRefresh: pullDownRefresh,
      pullUpLoad: pullUpLoad,
      startY: startY,
      freeScroll: freeScroll,
    }) as Object
    bscrollInstance.current = new BetterScroll(wrapperDom, _options)
    if (listenScroll) {
      bscrollInstance.current.on('scroll', (pos) => {
        scroll(pos)
      })
    }

    if (listenBeforeScroll) {
      bscrollInstance.current.on('beforeScrollStart', () => {
        beforeScrollStart()
      })
    }

    if (pullDownRefresh) {
      _initPullDownRefresh()
    }

    if (pullUpLoad) {
        _initPullUpLoad()
    }
  }
  const _initPullDownRefresh = () => {
    const scrollInstance = bscrollInstance.current
    scrollInstance.on('pullingDown', () => {
      setPullInfo({
        beforePullDown: false,
        isPullingDown: true,
        isPulling: true,
      })

      onPullingDown()
    })

    scrollInstance.on('scroll', (pos) => {
      if (pullInfo.beforePullDown) {
        //  this.bubbleY = Math.max(0, pos.y + this.pullDownInitTop)
        //  this.pullDownStyle = `top:${Math.min(pos.y + this.pullDownInitTop, 10)}px`
      } else {
        //  this.bubbleY = 0
      }

      if (isRebounding.current) {
        setPullInfo((prev) => ({
          pullDownStyle: { top: `${10 - (pullDownRefresh.stop - pos.y)}px` },
          ...prev,
        }))
      }
    })
  }
  const _initPullUpLoad = () => {
    const scrollInstance = bscrollInstance.current
    scrollInstance.on('pullingUp', (e) => {
      if (isPullUpToPageEnd.current) {
        setPullInfo((prev) => ({ isPullUpLoad: true, ...prev }))
        onPullingUp()
      }
    })
  }

 
    useImperativeHandle(bscrollListRef,() => ({
      disable() {
        bscrollInstance.current && bscrollInstance.current.disable()
      },
      enable() {
        bscrollInstance.current && bscrollInstance.current.enable()
      },
      refresh() {
        bscrollInstance.current && bscrollInstance.current.refresh()
      },
      scrollTo(){
        bscrollInstance.current &&
        // @ts-ignore
          bscrollInstance.current.scrollTo.apply(bscrollInstance.current, arguments)
      },
      scrollToElement(){
        bscrollInstance.current &&
        // @ts-ignore
         bscrollInstance.current.scrollToElement.apply(bscrollInstance.current, arguments)
      },
      destroy(){
        bscrollInstance.current && bscrollInstance.current.destroy()
      },
      forceUpdate(dirty){
        forceUpdate(dirty)
      },
      destroyBscrollList(){
        destroyBscrollList()
      }
    }))


  const refresh = () => {
    bscrollInstance.current && bscrollInstance.current.refresh()
  }
 

  // 加载数据后更新
  const forceUpdate = (dirty) => {
    setTimeout(() => {
      isPullUpToPageEnd.current = dirty
      if (pullDownRefresh && pullInfo.isPullingDown) {
        // this.pulling = false
        setPullInfo((prev) => ({ isPulling: false, ...prev }))
        _reboundPullDown().then(() => {
          _afterPullDown()
        })
      } else if (pullUpLoad) {
        //  this.isPullUpLoad = false
        setPullInfo((prev) => ({ isPullUpLoad: false, ...prev }))
        bscrollInstance.current.finishPullUp()
        refresh()
      } else {
        refresh()
      }
    })
  }
  const _reboundPullDown = () => {
    const { stopTime = 600 } = pullDownRefresh
    return new Promise((resolve) => {
      reboundPullDownTimerRef.current = setTimeout(() => {
        isRebounding.current = true
        setPullInfo((prev) => ({ isPullingDown: false, ...prev }))
        bscrollInstance.current.finishPullUp()
        bscrollInstance.current.finishPullDown()
        resolve('')
      }, stopTime)
    })
  }
  const _afterPullDown = () => {
    afterPullDownTimerRef.current = setTimeout(() => {
      setPullInfo((prev) => ({
        pullDownStyle: { top: `${pullInfo.pullDownInitTop}px` },
        beforePullDown: true,
        ...prev,
      }))
      isRebounding.current = false
      refresh()
    }, bscrollInstance.current.options.bounceTime)
  }
  const destroyBscrollList = () => {
    const scrollInstance = bscrollInstance.current
    scrollInstance && scrollInstance.destroy()
    bscrollInstance.current = null
    reboundPullDownTimerRef.current && clearTimeout(reboundPullDownTimerRef.current)
    reboundPullDownTimerRef.current = null
    afterPullDownTimerRef && clearTimeout(afterPullDownTimerRef.current)
    afterPullDownTimerRef.current = null
  }
  const getPullUpText = () => {
    const moreTxt = (pullUpLoad && pullUpLoad.txt && pullUpLoad.txt.more) || '加载更多'
    const noMoreTxt = (pullUpLoad && pullUpLoad.txt && pullUpLoad.txt.noMore) || '没有更多数据了'
    return isPullUpToPageEnd.current ? moreTxt : noMoreTxt
  }

  const getPullDownText = () => {
    return (pullDownRefresh && pullDownRefresh.txt) || '刷新成功'
  }

  const renderDefaultPullUpElement = () => {
    return (
      <>
        {pullUpLoad && (
          <div className={styles['pullup-wrapper']}>
            {pullInfo.isPullUpLoad ? (
              <div className={styles['before-trigger']}>
                <span>{getPullUpText()}</span>
              </div>
            ) : (
              <div className={styles['after-trigger']}>
                <img
                  className={styles['scroll-loading']}
                  src="data:image/gif;base64,"
                />
              </div>
            )}
          </div>
        )}
      </>
    )
  }
  const renderDefaultPullDownElement = () => {
    return (
      <>
        {/* {todo bubble} */}
        {pullDownRefresh && (
          <div className={styles['pulldown-wrapper']}>
            {pullInfo.isPulling ? (
              <div className={styles['before-trigger']}>
                <span>{getPullDownText()}</span>
              </div>
            ) : (
              <div className={styles['after-trigger']}>
                <img
                  className={styles['scroll-loading']}
                  src="data:image/gif;base64,"
                />
              </div>
            )}
          </div>
        )}
      </>
    )
  }

  return (
    <div className={styles["better-scroll__wrapper"]} style={{ overflow: 'hidden' }} ref={wrapperRef}>
      <div className={styles['scroll-content']}>
        <div>{children}</div>
        {pullUpElement ? pullUpElement:renderDefaultPullUpElement()}
      </div>
       {pullDownElement?pullDownElement:renderDefaultPullDownElement()}
    </div>
  )
}

export default BetterScrollList
