...
import CustomPulldownScrollList from './betterScrollPage/customPullDown';
...
const App = () => {
  const dicOfComponentArr: DicOfComponent[] = [
    ...
    { key: DemoType.async, value: <AsyncDataScrollList />, visible: false },
    {
      ...
      value: <CustomPulldownScrollList />,
      visible: false,
    },
  ];
  ...
  const { getParameterByName } = UseHistorySubscribe({
    cb: setActiveKeyAction,
  });
  ...
  const renderDemoOption = () => {
    return (
      <>
        {' '}
        <div className="option-btn" onClick={navigateTo(DemoType.simple)}>
          simple
        </div>
        <div className="option-btn" onClick={navigateTo(DemoType.async)}>
          async
        </div>
        <div
  ...
  return (
    <div style={{ height: '100%' }}>
      {activeKey
        ? dicOfComponentArr.find(item => item.key === activeKey)?.value
        : renderDemoOption()}
    </div>
  );
};
